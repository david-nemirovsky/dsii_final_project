...
```{r models, message = F, warning = F, eval = F}
...
```{r vip, message = F, warning = F, eval = F}
set.seed(37564)
vip(mod_boost, 
    method = "permute", 
    train = train_df,
...
```{r pred, message = F, warning = F, eval = F}
testna_df = 
...
```{r mod analysis, message = F, warning = F, eval = F}
cm_df = pred_boost %>% 
  as.data.frame() %>% 
  rename("survived" = ".") %>% 
...
```{r kaggle comp, message = F, warning = F, eval = F}
