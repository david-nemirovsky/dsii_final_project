...
res = resamples(list(ENET = mod_enet, MARS = mod_mars, KNN = mod_knn, BOOST = mod_boost, SVM_rad = mod_svm_radial ))
summary(res)
...
diff_df = 
  ...
  mutate(
    boost_surv = ifelse(pred_boost > 0.5, "N", "Y"), 
    mars_surv = ifelse(pred_mars > 0.5, "N", "Y"), 
...
test_pid = test_df$passenger_id
testX = test_df[-1]

bag_imp = preProcess(testX, method = "bagImpute")
imp_df = predict(bag_imp, testX)

test_res = predict(mod_boost, newdata = imp_df, type = "prob")[,1]

test_res = test_res %>% 
  as.data.frame() %>% 
  mutate(Survived = ifelse(test_res > 0.5, 0, 1))

res = cbind(test_pid, test_res$Survived)
