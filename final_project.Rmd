...
mod_mars = train(survived ~ ., 
                 ...
                 data = train_df, 
                 method = "earth",
                 tuneGrid = expand.grid(degree = 1:4, nprune = 8:16), 
                 metric = "ROC", 
                 trControl = ctrl)
...
#                tuneGrid = data.frame(k = seq(5, 105, by = 20)), 
#                trControl = ctrl)
...
res = resamples(list(GLM = mod_glm, MARS = mod_mars
               #, KNN = mod_knn
               ))

res_rf = resamples(list(Boost = mod_rf))
bwplot(res, metric = "ROC", main = "AUC for CV of Various Models")
```
